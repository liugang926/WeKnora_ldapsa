...
func (s *groupAccessDirectoryStub) ListGroups(_ context.Context, directoryID, _ string, offset, limit int) ([]*types.DirectoryGroup, error) {
...
func (s *groupAccessDirectoryStub) ListGroupMemberships(_ context.Context, groupID string) ([]*types.DirectoryGroupMembership, error) {
	return s.memberships[groupID], nil
}

func (s *groupAccessDirectoryStub) ListGroupEdges(context.Context, string) ([]*types.DirectoryGroupEdge, error) {
	return nil, nil
}

func (s *groupAccessDirectoryStub) GetIdentity(_ context.Context, id string) (*types.DirectoryIdentity, error) {
...
func (s *groupAccessRepoStub) ListTenantGroupRoleGrants(context.Context, uint64) ([]*types.TenantGroupRoleGrant, error) {
	return s.tenantGrants, nil
}

func (s *groupAccessRepoStub) ListResourceGroupGrants(context.Context, uint64, types.ResourceType, string) ([]*types.ResourceGroupGrant, error) {
	return s.resourceGrants, nil
}

func (s *groupAccessRepoStub) GetResourceAccessPolicy(context.Context, uint64, types.ResourceType, string) (*types.ResourceAccessPolicy, error) {
	return s.policy, nil
}
...
func (s *groupAccessServiceStub) UpsertResourceGroupGrant(_ context.Context, grant *types.ResourceGroupGrant) error {
	copy := *grant
	s.upserts = append(s.upserts, &copy)
	return nil
}

func (s *groupAccessServiceStub) SetResourceAccessPolicy(_ context.Context, policy *types.ResourceAccessPolicy) error {
	copy := *policy
	s.policy = &copy
	return nil
}
...
func (s *groupAccessKBRepoStub) GetKnowledgeBaseByIDAndTenant(_ context.Context, id string, tenantID uint64) (*types.KnowledgeBase, error) {
...
func groupAccessTestContext(method, path, body string, tenantID uint64) (*gin.Context, *httptest.ResponseRecorder) {
	...
	requestCtx = types.WithCaller(requestCtx, types.Caller{TenantID: tenantID, UserID: "admin", Role: types.TenantRoleAdmin})
...
func TestGroupAccessRejectsOwnerRoleForDirectoryGroup(t *testing.T) {
	...
	ctx, _ := groupAccessTestContext("POST", "/tenants/7/directory-groups", `{"directory_id":"d1","directory_group_id":"g1","role":"owner"}`, 7)
...
func TestGroupAccessResourceUpdatePreservesDirectoryOrigin(t *testing.T) {
	...
	directoryRepo := &groupAccessDirectoryStub{
		...
		groups: map[string][]*types.DirectoryGroup{"d1": {
			{ID: "g-new", DirectoryID: "d1", DisplayName: "New", Status: types.DirectoryObjectActive},
			{ID: "g-manual-old", DirectoryID: "d1", DisplayName: "Old", Status: types.DirectoryObjectActive},
			{ID: "g-directory", DirectoryID: "d1", DisplayName: "Synced", Status: types.DirectoryObjectActive},
		}},
		memberships: map[string][]*types.DirectoryGroupMembership{},
	}
	groupRepo := &groupAccessRepoStub{
		resourceGrants: []*types.ResourceGroupGrant{
			{ID: "manual", TenantID: 7, ResourceType: types.GroupResourceTypeKnowledgeBase, ResourceID: "kb1", DirectoryGroupID: "g-manual-old", Permission: types.ResourcePermissionRead, Origin: types.GrantOriginManual},
			{ID: "directory", TenantID: 7, ResourceType: types.GroupResourceTypeKnowledgeBase, ResourceID: "kb1", DirectoryGroupID: "g-directory", Permission: types.ResourcePermissionRead, Origin: types.GrantOriginDirectory},
		},
	}
	...
	ctx, recorder := groupAccessTestContext("PUT", "/group-access/knowledge_base/kb1", `{"mode":"restricted","grants":[{"directory_id":"d1","directory_group_id":"g-new","permission":"read"}]}`, 7)
	ctx.Params = gin.Params{{Key: "resource_type", Value: "knowledge_base"}, {Key: "resource_id", Value: "kb1"}}
	...
	if len(access.deleted) != 1 || access.deleted[0].DirectoryGroupID != "g-manual-old" || access.deleted[0].Origin != types.GrantOriginManual {
		t.Fatalf("expected only the stale manual grant to be deleted, got %#v", access.deleted)
	}
	if len(access.upserts) != 1 || access.upserts[0].DirectoryGroupID != "g-new" || access.upserts[0].Origin != types.GrantOriginManual {
