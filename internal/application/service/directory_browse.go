...
func (s *directoryRuntimeService) QueryGroupMembers(
	ctx context.Context,
...
func directoryGroupMembers(
	...
	for _, group := range snapshot.Groups {
		groups[group.ObjectGUID] = types.DirectoryObjectSummary{
	...
	result := &types.DirectoryGroupMembersResult{
		Group: group, Items: []types.DirectoryGroupMember{},
	...
	for _, edge := range snapshot.GroupMemberships {
		children[edge.ParentGroupGUID] = append(
			children[edge.ParentGroupGUID],
			edge.MemberGroupGUID,
		)
	...
	for _, membership := range snapshot.EffectiveMemberships {
		...
		if _, found := next[origin]; !found {
			return nil, fmt.Errorf(
				"%w: missing membership path",
				ldapdirectory.ErrIncompleteResults,
			)
		}
		...
		origins[membership.UserGUID] = append(
			origins[membership.UserGUID],
			types.DirectoryMembershipOrigin{
				Source: string(
					membership.Source,
				), OriginSource: string(originSource), Depth: len(path) - 1, Path: path,
			},
		)
	}
	for _, user := range snapshot.Users {
		memberOrigins := origins[user.ObjectGUID]
		if len(memberOrigins) == 0 ||
			!containsFold(
				query,
				user.DisplayName,
				user.SAMAccountName,
		...
		result.Items = append(
			result.Items,
			types.DirectoryGroupMember{DirectoryObjectSummary: types.DirectoryObjectSummary{
				...
				Email:             user.Email, Disabled: !user.Enabled,
			}, Origins: memberOrigins},
		)
	}
	...
	sort.Slice(
		result.ParentGroups,
		func(i, j int) bool { return directoryObjectLess(result.ParentGroups[i], result.ParentGroups[j]) },
	)
	sort.Slice(
		result.ChildGroups,
		func(i, j int) bool { return directoryObjectLess(result.ChildGroups[i], result.ChildGroups[j]) },
	)
