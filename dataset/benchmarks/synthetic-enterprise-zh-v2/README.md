...
应用内评估还应设置或记录 `EVALUATION_MAX_CONCURRENCY`，检查模型服务是否有 `429` 或检索/重排降级。旧版评估在一次 70 题并发运行中发生上述降级，虽显示任务成功，但不能作质量基线。修复版隔离应用在并发 2、指标口径 v2 下完成 70/70，40 道有证据题的 Recall=1.000，运行时段未见模型 429 或 API 回退；其对话模型仍是本机测试桩，不是生成答案质量验收。应用内 Parquet 评估不加载这里的虚构 persona 元数据，不能据此判定权限拒绝通过。任务和镜像来源详见 `docs/rag-baseline-acceptance.md`。
